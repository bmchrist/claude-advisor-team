...
analyses/{slug}/00_deal_materials/
  ...
  spv_deal_terms.md        <- hand-curated, deal-specific (e.g. quaise_energy)
```

Run `/ingest-materials "Company-or-slug"` after placing raw files in `raw/`
and before `/research`. RC reads `materials_digest.md`; advisors and Bear
consult `manifest.md` for routed exhibits — see `projectplan.md` item 3 for
the full digest + manifest convention and the `[DATA ROOM]` evidence tag
(per-stage manifest-reading behavior for RC/advisors/Bull/Bear is still
